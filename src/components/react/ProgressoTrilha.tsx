...
export default function ProgressoTrilha({ modulos, trilhaId }: { modulos: Modulo[]; trilhaId: string }) {
  ...
  useEffect(() => { const salvo = localStorage.getItem(chave); if (salvo) { try { setConcluidos(JSON.parse(salvo)); } catch { setConcluidos([]); } } else setConcluidos([]); }, [chave]);
  ...
  const exportar = () => { const blob = new Blob([JSON.stringify({ trilhaId, concluidos }, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `${trilhaId}-progresso.json`; document.body.appendChild(link); link.click(); document.body.removeChild(link); setTimeout(() => URL.revokeObjectURL(url), 1000); };
